...
/*
 * LZMA decompressor, based on version from Linux kernel, but heavily stripped
 * down, stream mode completely removed.
 */
...
/* Called once */
static inline void rc_init(struct rc *rc, char *buffer, long buffer_size)
{
...
static inline void rc_do_normalize(struct rc *rc)
{
	rc->range <<= 8;
	rc->code = (rc->code << 8) | *rc->ptr++;
}
...
static inline int rc_get_bit(struct rc *rc, uint16_t *p, int *symbol)
{
	if (rc_is_bit_0(rc, p)) {
	       ...
	       unsigned char *outbuf, long out_len, long *out_size)
{
	...
	if ((u8 *)(p + num_probs) > &free_mem_end)
		return -1;
