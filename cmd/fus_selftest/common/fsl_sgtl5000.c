...
#define SGTL_SWAP_UINT16_BYTE_SEQUENCE(x) ((x>>8) | (x<<8))
//#define SGTL_SWAP_UINT16_BYTE_SEQUENCE(x) (x)
...
status_t SGTL_Init(sgtl_handle_t *handle, sgtl_config_t *config)
{
    ...
    /* Set len of register address to 2 Byte */
    ret = i2c_set_chip_offset_len(handle->i2cHandle,2);
	if (ret) return ret;
#if 0
    ...
    ret = SGTL_WriteReg(handle, CHIP_ANA_POWER, 0x6AFF);
    if (ret) return ret;

    if (config->master_slave == 1) {
    	if (config->format.mclk_HZ > 17000000){
    		SGTL_ModifyReg(handle, CHIP_CLK_TOP_CTRL, 0xFFF7U, 0x0008U);
    	}
    	uint32_t PLL_Output_Freq;
    	if (config->format.sampleRate == 44100)
    		PLL_Output_Freq =  180633600;
    	else
    		PLL_Output_Freq =  196608000;

    	uint32_t int_div = PLL_Output_Freq/config->format.mclk_HZ;
    	uint32_t frac_div = (((PLL_Output_Freq*1000)/(config->format.mclk_HZ*1000))-int_div*1000)*2048;
    	frac_div = 0;
    	printf ("%d %d \n",int_div,frac_div);
    	SGTL_ModifyReg(handle, CHIP_PLL_CTRL, 0x07FFU, int_div<<11);
    	SGTL_ModifyReg(handle, CHIP_PLL_CTRL, 0xF800U, frac_div);

    }
    /* Set the data route */
    ret = SGTL_SetDataRoute(handle, config->route);
    if (ret) return ret;
    ...
    ret = SGTL_SetProtocol(handle, config->bus);
    if (ret) return ret;
    ret = SGTL_ConfigDataFormat(handle, config->format.mclk_HZ, config->format.sampleRate, config->format.bitWidth);
    if (ret) return ret;
    /* sclk valid edge */
...
status_t SGTL_ReadReg(sgtl_handle_t *handle, uint16_t reg, uint16_t *val)
{
    ...
    retval = dm_i2c_read(handle->i2cHandle,reg, (uint8_t*)&readValue, sizeof(readValue));
