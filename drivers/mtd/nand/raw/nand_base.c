 ...
 * the chip.
 */
static void nand_decode_id(struct nand_chip *chip, const struct nand_flash_dev *type)
{
	struct mtd_info *mtd = &chip->mtd;
...
int nand_detect(struct nand_chip *chip, int *maf_id,
		int *dev_id, const struct nand_flash_dev *type)
{
	struct mtd_info *mtd = &chip->mtd;
...
ident_done:
	...
	pr_info("NAND device: Manufacturer ID: 0x%02x, Chip ID: 0x%02x (%s %s)\n",
		*maf_id, *dev_id, manufacturer_desc->name,
#ifdef CONFIG_SYS_NAND_ONFI_DETECTION
